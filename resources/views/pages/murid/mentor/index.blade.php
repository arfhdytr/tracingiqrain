...
@push('styles')
    <style>
        ...
        .font-cursive-iwk { font-family: 'Tegak Bersambung_IWK', cursive !important; }
        .font-titan { font-family: 'Titan One', cursive !important; }
        .font-nanum { font-family: 'Nanum Myeongjo', serif !important; }

        .text-shadow-header { text-shadow: 0 2px 4px rgba(0, 0, 0, 0.25); }
        .text-shadow-popup-name { text-shadow: 0 4px 4px rgba(0, 0, 0, 0.25); }
        .text-shadow-popup-text { text-shadow: 0 2px 4px rgba(0, 0, 0, 0.25); }

        .hover-float:hover { transform: translateY(-5px) scale(1.02); }

        /* Styles Pop Up */
        .popup-stat-box {
            width: auto; min-width: 180px; height: auto; padding: 15px 25px;
            flex-shrink: 0; border-radius: 23px; background: #56B1F3;
            box-shadow: 0 2px 4px 0 rgba(0, 0, 0, 0.25);
            display: flex; align-items: center; justify-content: center; gap: 15px;
        }

        .popup-btn {
            width: auto; height: auto; padding: 10px 40px;
            flex-shrink: 0; border-radius: 9px; background: #F387A9;
        ...
        .popup-btn:disabled { background: #cccccc; cursor: not-allowed; }
    </style>
@endpush
...
@section('content')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-20 gap-x-10 justify-items-center">
                            @forelse($mentors as $index => $mentor)
                                ...
                                <div class="relative flex flex-col items-center cursor-pointer group hover-float w-[300px]"
                                    ...
                                    <div class="relative w-[300px] h-[300px] flex items-center justify-center mb-2">
                                        <img src="{{ asset('images/mentor/Mentor.webp') }}" alt="Frame" class="absolute w-full h-full object-contain z-0" style="{{ $vectorStyle }}">
                                        <img src="{{ asset('images/mentor/Centang.webp') }}" alt="Verified" class="absolute top-0 w-20 h-20 z-20 drop-shadow-md {{ $checkmarkClass }}">
                                        <div class="relative z-10 w-44 h-44 rounded-full overflow-hidden border-[5px] border-white shadow-inner bg-white">
                                            <img src="{{ $mentor->user->avatar_url }}" alt="{{ $mentor->nama_lengkap }}" class="w-full h-full object-cover">
                                        </div>
                                    </div>
                            ...
                            @empty
                                <div class="col-span-full text-center py-10">
                                    <p class="font-cursive-iwk text-4xl text-white/80">Belum ada mentor yang tersedia saat ini.</p>
                                </div>
                            @endforelse
                        </div>
                    </div>
    ...
    {{-- POPUP MODAL DETAIL --}}
    {{-- ======================= --}}
    <div id="mentorModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/60 backdrop-blur-sm transition-opacity duration-300">
        <div class="relative w-[95%] max-w-[1000px] flex items-center justify-center transition-all duration-300 transform scale-95 opacity-0" id="modalContent">
            
            {{-- Background Popup --}}
            <img src="{{ asset('images/mentor/Popup.webp') }}" alt="Popup BG" class="w-full h-auto object-contain drop-shadow-2xl relative z-0">
            ...
            <div class="absolute inset-0 z-10 flex flex-col p-6 md:p-12">
                <div class="flex flex-col md:flex-row w-full flex-grow items-center justify-center">
                    ...
                    <div class="w-full md:w-[30%] flex items-center justify-center h-full mb-4 md:mb-0">
                        <div class="relative z-10 w-32 h-32 md:w-50 md:h-50 rounded-full border-[5px] border-white shadow-lg bg-white overflow-hidden translate-x-15 translate-y-15">
                    ...
                    <div class="w-full md:w-[70%] flex flex-col items-center justify-center pl-0 md:pl-6">
                        <h2 id="modal-mentor-name" class="font-titan text-[32px] md:text-[45px] text-[#AC3F61] text-center leading-none mb-1 text-shadow-popup-name">
                            Kak Nama
                        </h2>

                        {{-- KELAS (Target Lengkung Frasa 1) --}}
                        <p id="modal-mentor-class" class="text-[28px] md:text-[40px] text-center leading-tight mb-4 md:mb-6 text-shadow-popup-text">
                            {{-- Diisi JS --}}
                        </p>
    ...
    <div id="customAlertModal" class="fixed inset-0 z-[60] hidden items-center justify-center bg-black/60 backdrop-blur-sm transition-opacity duration-300">
        <div class="relative bg-white rounded-3xl p-8 max-w-sm w-full mx-4 shadow-2xl transform scale-95 opacity-0 transition-all duration-300 border-4 border-pink-300 text-center" id="customAlertContent">
            <div class="mb-4 animate-bounce">
                <img id="customAlertIcon" src="{{ asset('images/icon/checklist.webp') }}" alt="Icon" class="w-24 h-auto mx-auto">
            </div>
